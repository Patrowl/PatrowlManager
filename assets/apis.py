...
def get_asset_trends_api(request, asset_id):
    ...
    for i in range(0, nb_ticks):
        ...
        for f in asset.finding_set.filter(created_at__lte=enddate).exclude(Q(status='falsepositive') | Q(status='duplicate')| Q(status='closed')| Q(status='mitigated')).values("severity", "status"):
