...
def get_findings_stats_api(request):
    ...
    data = {
        ...
        "nb_low": findings.filter(severity="low").exclude(status='false-positive').count(),
        "nb_medium": findings.filter(severity="medium").exclude(status='false-positive').count(),
        "nb_high": findings.filter(severity="high").exclude(status='false-positive').count(),
        "nb_critical": findings.filter(severity="critical").exclude(status='false-positive').count(),
