...
class ScanDefinition(models.Model):
    ...
    dynassetgroups_list = models.ManyToManyField('assets.DynamicAssetGroup', blank=True)
    ...
    def to_dict(self):
        data = model_to_dict(self, exclude=["assets_list", "assetgroups_list", "dynassetgroups_list"])
        ...
        data.update({"dynassetgroups_list": [model_to_dict(a, fields=["id", "name"]) for a in self.dynassetgroups_list.all()]})
